<button type="button" onclick="openModal('deleteModal{{ $item->id }}')"
    class="bg-red-500 hover:bg-red-800 rounded-full p-2 transition duration-200">
    ...
    <div class="bg-white p-6 rounded-lg w-96 relative">
        ...
        <form method="POST" action="{{ route('produk.destroy', $item->id) }}">
            ...
            <div class="flex justify-end gap-2">
                <button type="button" onclick="closeModal('deleteModal{{ $item->id }}')"
                    class="px-4 py-2 bg-gray-300 hover:bg-gray-400 rounded">Batal</button>
                <button type="submit" class="px-4 py-2 bg-red-600 text-white hover:bg-red-700 rounded">Yakin</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    function openModal(id) {
        ...
        if (modal) {
            ...
            modal.classList.add('flex'); // agar modal muncul di tengah layar
        }
    }
